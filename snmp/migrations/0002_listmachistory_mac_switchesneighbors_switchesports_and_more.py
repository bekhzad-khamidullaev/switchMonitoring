...
def deduplicate_switch_hostname_ip(apps, schema_editor):
    Switch = apps.get_model('snmp', 'Switch')
    duplicates = (
        Switch.objects.values('hostname', 'ip')
        .annotate(row_count=Count('id'))
        .filter(row_count__gt=1)
    )
    for item in duplicates:
        rows = Switch.objects.filter(hostname=item['hostname'], ip=item['ip']).order_by('id')
        keep = rows.first()
        if keep is not None:
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(deduplicate_switch_hostname_ip, migrations.RunPython.noop),
