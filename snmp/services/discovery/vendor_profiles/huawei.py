...
class HuaweiVendorProfile(BaseVendorProfile):
    name = "huawei"
    ...
    def supports(
        ...
        return (
            vendor == "huawei"
            or sys_object_id.startswith("1.3.6.1.4.1.2011.")
            or "huawei" in sys_descr
        )
    ...
    def enrich_interfaces(self, interfaces: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in interfaces:
            ...
            if current.get("is_optical") is False:
                # Huawei often uses XGigabit/25GE naming even if ifType isn't enough.
                current["is_optical"] = (
                    if_type in {"117"}
