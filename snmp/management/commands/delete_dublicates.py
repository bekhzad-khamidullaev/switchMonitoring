...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        duplicate_hostname = ManagedDevice.objects.values('hostname').annotate(count=Count('hostname')).filter(count__gt=1)
        ...
        for hostname in duplicate_hostname:
            ...
            duplicate_hosts = ManagedDevice.objects.filter(hostname=hostname).order_by('-hostname')[1:]
