...
def api_device_onboard(request):
    ...
    switch_id = serializer.validated_data.get('switch_id')
    switch = None

    if switch_id:
        switch = Switch.objects.filter(pk=switch_id).first()
        if not switch:
            return Response({'detail': 'Switch not found'}, status=status.HTTP_404_NOT_FOUND)
        ip = ip or switch.ip
    ...
    device, _ = Device.objects.get_or_create(
        ...
        defaults={
            ...
            'status': bool(switch.status) if switch else False,
        },
    )
    if switch and not device.switch_id:
...
def api_device_discover(request, device_id):
    device = Device.objects.filter(pk=device_id).select_related('switch').first()
    if not device:
        return Response({'detail': 'Device not found'}, status=status.HTTP_404_NOT_FOUND)
    if not user_can_access_device(request.user, device):
        return Response({'detail': 'Forbidden'}, status=status.HTTP_403_FORBIDDEN)

    community = request.data.get('community') or (device.switch.snmp_community_ro if device.switch_id else 'public')

    try:
        result = run_device_discovery(ip=str(device.ip), community=community, switch=device.switch)
...
def api_subscription_update(request, subscription_id):
    subscription = (
        ...
        .select_related('device', 'device__switch', 'binding')
        .first()
    )
    if not subscription:
